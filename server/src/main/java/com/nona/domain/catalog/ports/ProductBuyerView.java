 ...
 * 清单——商品详情页元素 + SKU 规格联动数据 + 下单前置读字段一次成型）：
 * <ul>
 *     <li>主体：名称/描述/图片/自定义属性（商品基本信息完整展示）；</li>
 *     <li>规格联动：规格维度结构（维度名 + 可选值——前端规格选择区）+ SKU
 *         级精确价与可售量（规格项联动/无库存置灰/价格与库存随动
 *         ——按 skuId zip 可售，缺行 0 置灰）；</li>
 *     <li><b>运费说明</b>：运费模板规则概要（恒非空——绑定模板概要；未绑定/绑定
 *         悬挂回退店铺默认模板概要，非空设计 §2.5。运费试算不在本读面，结算页
 *         试算由订单域经运费计算器完成）；</li>
 *     <li>店铺卡片：店名/logo（店铺卡片展示；店铺为 global 聚合，买家
 *         读无需放行）。</li>
 * </ul>
 ...
 * @param freight           运费模板规则概要（恒非空：绑定概要；未绑定/悬挂回退默认概要）
...
public record ProductBuyerView(
    ...
    /**
     * 运费模板规则概要（详情运费区展示；恒非空——绑定模板概要；商品未绑定/
     * 绑定悬挂时回退店铺默认模板概要，前端按有运费模板呈现）。
     *
