...
/**
 * 入驻申请审核通过领域事件（跨上下文契约，通知旁路）：开店编排（创建店铺
 * 聚合 + 账号-店铺关联）由应用层用例在同事务内完成，本事件仅负责发布
 * （进程内总线，同步分发），供日志/追踪等旁路消费。
 * <p>
 * payload 携带创建店铺所需的最小定位信息（申请 ID / 提交实体 /
 * 店铺名），旁路消费方需要更多资料时可经申请 ID 回查申请聚合。
 *
 * @author nona9961
 */
