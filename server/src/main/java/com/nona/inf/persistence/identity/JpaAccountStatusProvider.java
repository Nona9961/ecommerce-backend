 ...
 * <p>
 * 单表语义（领域模型 M5）：账号只有一张表，type 列区分买家/商家——
 * 角色由 type 定向推导（BUYER → [BUYER]、SELLER → [SELLER]），商家附带读取
 * account_shop_rel 填充 shopIds（M10，一期恒 1 个）；<b>平台运营（admin）账号
 * 无落点</b>（RBAC 属 WU-10/Phase-II），本表不存在 ADMIN 类型，portal=ADMIN
...
public class JpaAccountStatusProvider implements AccountStatusProvider {
     ...
     * account_shop_rel 填充 shopIds（M10，一期恒 1 个）；域状态映射安全链状态
