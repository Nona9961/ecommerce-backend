 ...
 * 已发货 → 已完成 + 主单派生 + 完成事件发布，与买家确认收货
 * 共用同一完成迁移，触发源由调用方语义区分（收货超时由超时
 * 调度引擎触发，本类只承载签收触发面）。
 * <p>
 * 装配形态（对齐跨域事件消费链路契约）：
 * <ul>
 ...
 *         AFTER_COMMIT 投递，与确认/超时消费位同一机制）；</li>
 *     <li><b>提权写段</b>：跨租户写（子单 tenant=shopId 状态推进）在
 *         {@link TenantPrivilege#elevatedInTransaction} 内整体执行——
 *         监听线程无请求视角（系统触发），写 tenant 表须提权 + PO
 *         租户由写门禁注入/显式承载（读放行与写放行合一）。
 *     </li>
 * </ol>
 * 装配声明：本类依赖订单侧仓储（子单装载）与订单门面——均已接线：
 * Spring 注册（{@code @Service}）就位；单测以构造器直接装配。
 * <p>
